    ...
    for (int idx = 0; idx < ngpus; ++idx) {
        ...
        for (UINT rep = 0; rep < max_repeat; ++rep) {
            ...
            test_state = get_expanded_eigen_matrix_with_identity(
                         ...
                         test_state;
            state_equal_gpu(
                state, test_state, dim, "single Pauli gate", stream_ptr, idx);
        }
    ...
    for (int idx = 0; idx < ngpus; ++idx) {
        ...
        for (UINT rep = 0; rep < max_repeat; ++rep) {
            ...
            state_equal_gpu(state, test_state, dim,
                "single rotation Pauli gate", stream_ptr, idx);
        }
        release_quantum_state_host(state, idx);
        release_cuda_stream_host(stream_ptr, 1, idx);
    }
    ...
    for (int idx = 0; idx < ngpus; ++idx) {
        ...
        for (UINT rep = 0; rep < max_repeat; ++rep) {
            ...
            test_state =
                ...
                test_state;
            state_equal_gpu(state, test_state, dim,
                "multi Pauli rotation whole gate", stream_ptr, idx);
            ...
            test_state =
                ...
                test_state;
            state_equal_gpu(state, test_state, dim,
                "multi Pauli rotation partial gate", stream_ptr, idx);
        }
        release_quantum_state_host(state, idx);
        release_cuda_stream_host(stream_ptr, 1, idx);
    }
